...
- **国际版 Global**：Claude、ChatGPT、Gemini、Manus、Perplexity
- **国内版**：DeepSeek、GLM、MiniMax、豆包、Kimi、文心一言、Qwen、即梦、扣子

随时切换区域——每个区域上次使用的服务会被记住。

### 插件

- 主应用极度轻量，仅保留 webview 容器、导航栏、基础设置，不内置任何可选功能
- 用户按需下载插件，自己决定加载哪些功能，互不干扰
- 插件从 GitHub 统一发布，每次迭代上传新版本，客户端自动感知
- 插件下载地址：https://github.com/unhurriedly/OmniAi-Desktop/tree/main/Plugins
